...
import { X, ClipboardCheck, Send, Loader2, Volume2, Download } from 'lucide-react';
...
const AudioPlayer: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div className="absolute top-2 right-2 z-10 flex items-center gap-2">
            <button onClick={handleGenerateAudio} disabled={isGeneratingAudio} className="p-1.5 bg-gray-800/70 rounded-full text-white hover:bg-gray-700/90 transition-colors" aria-label="Ouvir rotina">
                {isGeneratingAudio ? <Loader2 className="w-4 h-4 animate-spin" /> : <Volume2 size={16} />}
            </button>
            {audioUrl && (
                <a href={audioUrl} download="rotina.wav" className="p-1.5 bg-gray-800/70 rounded-full text-white hover:bg-gray-700/90 transition-colors" aria-label="Baixar áudio">
                    <Download size={16} />
                </a>
            )}
